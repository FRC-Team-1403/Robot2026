...
public class Vision extends SubsystemBase {
  ...
  private Pose3d combinePoses(List<WeightedPose> weightedPoses) {
    ...
    double totalWeight = weightedPoses.stream().mapToDouble(wp -> wp.weight).sum();

    double avgX =
        weightedPoses.stream().mapToDouble(wp -> wp.pose.getX() * wp.weight).sum() / totalWeight;

    double avgY =
        weightedPoses.stream().mapToDouble(wp -> wp.pose.getY() * wp.weight).sum() / totalWeight;

    double avgZ =
        weightedPoses.stream().mapToDouble(wp -> wp.pose.getZ() * wp.weight).sum() / totalWeight;
    ...
    return new Pose3d(avgX, avgY, avgZ, new Rotation3d(0, 0, avgAngleRad));
  }
